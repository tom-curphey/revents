...
// Query to get the data from Auth Users
const query = ({ auth }) => {
	// You need to pass an array to a query
	return [
		{
			collection: 'users',
			doc: auth.uid,
...
class PhotosPage extends Component {
	...
	cropImage = () => {
		// If there is now image that has been uploaded just return back to the parent function
		if (
			typeof this.refs.cropper.getCroppedCanvas() ===
			'undefined'
		) {
			return;
		}
					...
					{photos &&
						filteredPhotos.map((photo) => (
							<Card key={photo.id}>
								...
								>
									<Button
										onClick={this.handleSetMainPhoto(
											photo
										)}
										basic
										color="green"
